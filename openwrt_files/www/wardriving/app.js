...
function toggleBrowserGPS(){
  ...
  if(browserGpsActive){
    ...
    watchId=navigator.geolocation.watchPosition(p=>{window.carLat=p.coords.latitude;window.carLng=p.coords.longitude;carMarker.setLatLng([window.carLat,window.carLng]);if(firstLoc){map.setView([window.carLat,window.carLng],16);firstLoc=false;}else if(followMode){map.panTo([window.carLat,window.carLng],{animate:true,duration:0.3});}window.nmeaBuffer = window.nmeaBuffer || [];window.nmeaBuffer.push(generateNMEA(p));// Bound the buffer: drop oldest entries if the GPS push is failing// repeatedly (network down, CGI capped at 8KB). 32 GPRMC/GGA pairs is// ~8 minutes of 4-second flushes, enough to ride out a transient// outage without unbounded growth.window.GPS_BUFFER_MAX=window.GPS_BUFFER_MAX||32;if(window.nmeaBuffer.length>window.GPS_BUFFER_MAX)window.nmeaBuffer.splice(0,window.nmeaBuffer.length-window.GPS_BUFFER_MAX);window.lastGpsPush = window.lastGpsPush || 0;let now = Date.now();if(now - window.lastGpsPush > 4000){fetch(apiUrl('gps_push'),{method:'POST',body:window.nmeaBuffer.join('')}).then(()=>{window.nmeaBuffer=[];window.lastGpsPush=now;}).catch(()=>{/* keep buffer, will retry on next interval */});}},()=>{document.getElementById('chkBrowserGPS').checked=false;browserGpsActive=false;updateMapButtons();window.nmeaBuffer=[];window.lastGpsPush=0;},{enableHighAccuracy:window.GPS_HIGH_ACCURACY||false,maximumAge:1000,timeout:10000});
...
// HISTORY
function loadHistory(){fetch(apiUrl('history')).then(r=>r.json()).then(d=>{document.getElementById('stHistTotal').innerText=d.total;document.getElementById('stHistWpa3').innerText=d.wpa3;document.getElementById('stHistHs').innerText=d.hs;document.getElementById('stHistDays').innerText=d.days;let h='';d.sessions.forEach(s=>{h+='<div style="padding:8px;border:1px solid var(--border);border-radius:6px;margin-bottom:4px;display:flex;justify-content:space-between"><span>📅 '+esc(s.date)+'</span><span>'+esc(s.count)+' nets</span></div>';});document.getElementById('historyList').innerHTML=h||'<span style="color:var(--c-dim)">No data</span>';});}
