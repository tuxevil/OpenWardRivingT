...
# Default to a clearly non-routable placeholder so deployments fail loud rather
# than silently targeting a wrong host. Always override GPU_HOST/GPU_URL.
...
echo "== Backups =="
ssh_cmd "$ROUTER_HOST" "set -eu; b=/var/backups/openwardrivingt/openwardrivingt_deploy_backup_$STAMP; mkdir -p \"\$b\"; cp -f /www/cgi-bin/wardriving_api \"\$b/\" 2>/dev/null || true; cp -f /usr/bin/wardriving_core.sh \"\$b/\" 2>/dev/null || true; cp -f /usr/bin/wardriving_clients.sh \"\$b/\" 2>/dev/null || true; cp -f /usr/bin/wardriving_replay.sh \"\$b/\" 2>/dev/null || true; cp -f /etc/init.d/wardriving \"\$b/\" 2>/dev/null || true; cp -f /www/wardriving/index.html /www/wardriving/app.css /www/wardriving/app.js \"\$b/\" 2>/dev/null || true; cp -rf /usr/lib/wardriving \"\$b/\" 2>/dev/null || true; cp -f /etc/wardriving_remote_secret \"\$b/\" 2>/dev/null || true; echo \"\$b\""
ssh_cmd "$GPU_HOST" "set -eu; b=/var/backups/openwardrivingt/openwardrivingt_gpu_deploy_backup_$STAMP; mkdir -p \"\$b\"; cp -f /var/lib/openwardrivingt/gpu_server.py \"\$b/\" 2>/dev/null || true; cp -f /var/lib/openwardrivingt/run_hashcat.sh \"\$b/\" 2>/dev/null || true; cp -f /etc/systemd/system/wardriving_gpu.service \"\$b/\" 2>/dev/null || true; echo \"\$b\""
...
echo "== Deploy GPU =="
copy_gpu server_files/gpu_server.py /var/lib/openwardrivingt/gpu_server.py
copy_gpu server_files/run_hashcat.sh /var/lib/openwardrivingt/run_hashcat.sh
copy_gpu server_files/wardriving_gpu.service /etc/systemd/system/wardriving_gpu.service
ssh_cmd "$GPU_HOST" "set -eu; chmod +x /var/lib/openwardrivingt/run_hashcat.sh; sed -i '/^Environment=OWRT_GPU_SHARED_SECRET=/d;/^Environment=OWRT_ROUTER_TOKEN=/d;/^Environment=OWRT_ROUTER_IP=/d' /etc/systemd/system/wardriving_gpu.service; sed -i '/^\\[Service\\]/a Environment=OWRT_ROUTER_IP=192.168.1.1\nEnvironment=OWRT_ROUTER_TOKEN=$ROUTER_TOKEN\nEnvironment=OWRT_GPU_SHARED_SECRET=$REMOTE_SECRET' /etc/systemd/system/wardriving_gpu.service; systemctl daemon-reload; systemctl restart wardriving_gpu; systemctl is-active wardriving_gpu"
