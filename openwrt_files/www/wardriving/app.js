...
function toggleHeatmap(){if(document.getElementById('chkHeatmap').checked){fetch('/cgi-bin/wardriving_api?action=heatmap_data').then(r=>r.json()).then(d=>{if(d.length>0)heatLayer=L.heatLayer(d,{radius:25,blur:15,maxZoom:17}).addTo(map);});}else{if(heatLayer){map.removeLayer(heatLayer);heatLayer=null;}}}
function startReplay(){if(replayTimer){clearInterval(replayTimer);replayTimer=null;return;}fetch('/cgi-bin/wardriving_api?action=map_data').then(r=>r.json()).then(d=>{if(!d.nmea_b64)return;let co=[],ls=atob(d.nmea_b64).split('\n');ls.forEach(l=>{let r=parseNMEARMC(l);if(r)co.push([r.lat,r.lon]);});if(!co.length)return;let i=0;replayTimer=setInterval(()=>{if(i>=co.length){clearInterval(replayTimer);replayTimer=null;return;}map.setView(co[i],16);carMarker.setLatLng(co[i]);i++;},500);});}
...
function toggleBrowserGPS(){
  ...
  if(browserGpsActive){
    ...
    watchId=navigator.geolocation.watchPosition(p=>{window.carLat=p.coords.latitude;window.carLng=p.coords.longitude;carMarker.setLatLng([window.carLat,window.carLng]);if(firstLoc){map.setView([window.carLat,window.carLng],16);firstLoc=false;}else if(followMode){map.panTo([window.carLat,window.carLng],{animate:true,duration:0.3});}window.nmeaBuffer = window.nmeaBuffer || [];window.nmeaBuffer.push(generateNMEA(p));window.lastGpsPush = window.lastGpsPush || 0;let now = Date.now();if(now - window.lastGpsPush > 4000){fetch('/cgi-bin/wardriving_api?action=gps_push',{method:'POST',body:window.nmeaBuffer.join('')});window.nmeaBuffer = [];window.lastGpsPush = now;}},()=>{document.getElementById('chkBrowserGPS').checked=false;browserGpsActive=false;updateMapButtons();},{enableHighAccuracy:window.GPS_HIGH_ACCURACY||false,maximumAge:1000,timeout:10000});
...
// CAPTURES
function loadFiles(){apiJson('list_files').then(d=>{let h='';if(!d||!d.length){document.getElementById('fileList').innerHTML='<span style="color:var(--c-dim)">No captures yet.</span>';return;}d.forEach(f=>{let name=String(f.name||'');h+='<div class="file-item"><div class="fi-name">'+esc(name)+'</div><div class="fi-nets">'+esc(f.nets)+'</div><div class="fi-meta">'+esc(f.size)+'<br>'+esc(f.date)+'</div><button class="btn small stop" onclick="deleteFile(\''+encodeURIComponent(name)+'\')">✕</button></div>';});document.getElementById('fileList').innerHTML=h;}).catch(e=>{setHtml('fileList',dim('files error: '+(e.message||'unknown')));});}
...
function loadHW(){fetch('/cgi-bin/wardriving_api?action=get_hw').then(r=>r.json()).then(d=>{let s=document.getElementById('selLED');s.innerHTML='';if(d.leds)d.leds.forEach(l=>{let o=document.createElement('option');o.value=l;o.text=l;if(l===d.current_led)o.selected=true;s.appendChild(o);});document.getElementById('selBtn').value=(d.current_button==='wifi'?'wifi':'wps');syncModeUi(d.mode||'passive');});}
function saveHW(){let l=document.getElementById('selLED').value,b=document.getElementById('selBtn').value,m=document.getElementById('selMode').value;fetch('/cgi-bin/wardriving_api?action=set_hw&led='+encodeURIComponent(l)+'&btn='+b+'&mode='+m).then(r=>r.json()).then(d=>{syncModeUi(m);if(d.status==='saved')showToast('Hardware saved');});}
function saveWigle(){let t=document.getElementById('wigleToken').value;if(!t)return;fetch('/cgi-bin/wardriving_api?action=save_wigle_token',{method:'POST',body:t}).then(r=>r.json()).then(d=>{if(d.status==='ok'){document.getElementById('wigleStatus').innerText='Token Saved!';setTimeout(()=>document.getElementById('wigleStatus').innerText='',3000);}});}
function uploadWigle(){document.getElementById('wigleStatus').innerText='Uploading...';fetch('/cgi-bin/wardriving_api?action=wigle_upload').then(r=>r.json()).then(d=>{document.getElementById('wigleStatus').innerText=d.success?'✓ Uploaded!':'Error: '+(d.error||'Unknown');}).catch(()=>{document.getElementById('wigleStatus').innerText='Network Error!';});}
function downloadBBox(){if(!map)return;let b=map.getBounds(),zMin=document.getElementById('dlZoomMin').value||12,zMax=document.getElementById('dlZoomMax').value||16;let n=b.getNorth(),s=b.getSouth(),e=b.getEast(),w=b.getWest();document.getElementById('bboxStatus').innerText='Starting...';fetch('/cgi-bin/wardriving_api?action=download_bbox&n='+n+'&s='+s+'&e='+e+'&w='+w+'&z1='+zMin+'&z2='+zMax).then(r=>r.json()).then(d=>{if(d.status==='started'){document.getElementById('bboxStatus').innerText='Downloading...';if(dlTimer)clearInterval(dlTimer);dlTimer=setInterval(()=>{fetch('/cgi-bin/wardriving_api?action=download_status').then(rr=>rr.json()).then(dd=>{if(dd.status==='DONE'){document.getElementById('bboxStatus').innerText='✓ Done!';clearInterval(dlTimer);}});},3000);}}).catch(()=>{document.getElementById('bboxStatus').innerText='Error';});}
function uploadTiles(){let f=document.getElementById('tilesFile').files[0];if(!f)return;document.getElementById('uploadStatus').innerText='Uploading...';fetch('/cgi-bin/wardriving_api?action=upload_tiles',{method:'POST',body:f}).then(r=>r.json()).then(d=>{document.getElementById('uploadStatus').innerText=d.status==='ok'?'✓ Installed!':'Error';setTimeout(()=>{if(map)map.invalidateSize();},1000);}).catch(()=>{document.getElementById('uploadStatus').innerText='Error';});}
...
function removeTarget(m){fetch('/cgi-bin/wardriving_api?action=remove_target&mac='+m).then(()=>loadTargets());}
function checkTargets(){if(!isRunning||!targetMacs.length)return;fetch('/cgi-bin/wardriving_api?action=check_targets').then(r=>r.json()).then(d=>{if(d&&d.length)d.forEach(n=>{if(!alertedTargets.includes(n.mac)){alertedTargets.push(n.mac);if(audioEnabled&&'speechSynthesis' in window)window.speechSynthesis.speak(new SpeechSynthesisUtterance('Target detected! '+n.ssid));}});});}
loadTargets();setInterval(checkTargets,10000);

// HISTORY
function loadHistory(){fetch('/cgi-bin/wardriving_api?action=history').then(r=>r.json()).then(d=>{document.getElementById('stHistTotal').innerText=d.total;document.getElementById('stHistWpa3').innerText=d.wpa3;document.getElementById('stHistHs').innerText=d.hs;document.getElementById('stHistDays').innerText=d.days;let h='';d.sessions.forEach(s=>{h+='<div style="padding:8px;border:1px solid var(--border);border-radius:6px;margin-bottom:4px;display:flex;justify-content:space-between"><span>📅 '+esc(s.date)+'</span><span>'+esc(s.count)+' nets</span></div>';});document.getElementById('historyList').innerHTML=h||'<span style="color:var(--c-dim)">No data</span>';});}
